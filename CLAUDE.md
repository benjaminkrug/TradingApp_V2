...
**Zweck dieser Datei:** Du (Claude) liest diese Datei automatisch beim Start jeder neuen Session in diesem Repo. Sie fasst den kompletten Stand zusammen, damit eine neue Session (z. B. am PC des Nutzers, mit echtem Netzwerkzugriff) sofort weiterarbeiten kann, ohne den gesamten bisherigen Chatverlauf zu kennen. Zuletzt aktualisiert: 24.09.2026, nach dem News-Volumen-Pilot (`NEWS_VOLUME_PILOT_PROTOCOL.md`) — Ergebnis: **kein Signal** (noch eindeutiger als beim Stimmungs-Pilot, t≈0 auf beiden Seiten). Weder GDELT-Stimmung noch GDELT-Nachrichten-Volumen verbessern die Übernacht-Auswahlstrategie als Verfeinerung. Die Übernacht-Auswahlstrategie selbst (siehe `OVERNIGHT_SELECTION_PROTOCOL.md`) bleibt unverändert der beste bisherige Kandidat, mit wichtigen Einschränkungen (K8 ungelöst, K4 nur knapp).
...
**Test-Stand (aktualisiert 24.09.2026, am PC verifiziert):** 308 Tests, alle grün. Zuwachs gegenüber den 305 vom 23.09.: GDELT-Client um `timelinevol`-Modus (Nachrichten-Volumen, `VolumeDay`/`fetch_volume_timeline`) erweitert, dieselbe Chunking/Backoff-Infrastruktur wiederverwendet. Die alten "217 laufen, 7 übersprungen"-Zahlen galten nur für die alte Sandbox ohne `pip install`.
...
NEWS_VOLUME_PILOT_PROTOCOL.md    Vorregistriertes + Ergebnis-Nachtrag für den GDELT-Nachrichten-Volumen-Pilot (24.09.2026) — gleiches 24-Symbol-Universum — **kein Signal** (t≈0 auf beiden Seiten, noch eindeutiger als beim Stimmungs-Pilot)
...
- **23.09.2026: News-Stimmungs-Pilot (GDELT) liefert KEIN verwertbares Signal** — s. `NEWS_SENTIMENT_PILOT_PROTOCOL.md`. Getestet: Verbessert eine zusätzliche Filterung der Übernacht-Auswahl nach relativer GDELT-Nachrichtenstimmung das Ergebnis? 24-Symbol-Pilot, 2 Jahre, 24/24 Symbole vollständig geladen. Ergebnis: Aktien mit *negativerer* Stimmung schnitten besser ab als die mit positiverer (Gegenteil der Hypothese), aber nicht signifikant (t=1,76, p=0,065 — knapp über 0,05, und weit über der bei 31 Konfigurationen insgesamt nötigen Bonferroni-Schwelle von ~0,0016). Nebenbei ein echter Bug gefunden und behoben: `_get_with_backoff` in `app/data/providers/gdelt.py` fing nur HTTP-429-Fehler ab, keine reinen Verbindungs-Timeouts — ein einzelner Netzwerk-Hänger brach den ganzen Mehr-Symbol-Abruf ab (mehrfach bei DELL/NVR beobachtet, jetzt mit Regressionstests behoben).
- **Neu, 24.09.2026: News-Volumen-Pilot (GDELT) liefert ebenfalls KEIN Signal — noch eindeutiger als die Stimmung** — s. `NEWS_VOLUME_PILOT_PROTOCOL.md`. Nutzer-Idee: statt/zusätzlich zu Stimmung könnte reines Nachrichten-*Volumen* (wie viele Artikel an einem Tag über eine Firma erscheinen, GDELT-Modus `timelinevol`) ein nützliches Auswahlsignal sein. Getestet als Verfeinerung der bereits nach Handelsvolumen selektierten Top-20-Aktien, gleiches 24-Symbol-Universum/2-Jahre-Fenster wie der Stimmungs-Pilot (keine neuen Kursdaten nötig). Ergebnis: obere Hälfte (hohes Nachrichten-Volumen) t=+0,12, p=0,89; untere Hälfte t=-0,07, p=0,92 — beide praktisch bei Null, kein erwähnenswerter Unterschied zwischen den Gruppen. Wahrscheinlicher Grund: Nachrichten-Volumen und Handelsvolumen korrelieren stark (gemeinsame Ursache), daher bleibt innerhalb einer schon volumen-vorselektierten Gruppe kaum zusätzlicher Erklärungswert übrig. **Wichtig:** Das ist ein Test von Nachrichten-Volumen als *Verfeinerung*, nicht als eigenständigem Auswahlsignal über den ganzen Markt (die ursprüngliche, größere Nutzer-Idee) — dieser härtere, teurere Test wurde bewusst noch nicht gemacht.
- **Konsequenz aus beiden GDELT-Piloten: Weder Stimmungs- noch Volumen-Filterung mit GDELT wird weiterverfolgt** — die reine Handelsvolumen-Auswahl bleibt der bessere Ausgangspunkt. **Nächster sinnvoller Schritt bleibt: K8 lösen (Databento Security Master, 199 USD/Monat — siehe `SELECTION_LAYER_PROTOCOL.md` für die Kostendiskussion) oder weitere Robustheitsprüfung der Übernacht-Auswahl, bevor an Forward-Test/echtes Kapital überhaupt zu denken ist.**
